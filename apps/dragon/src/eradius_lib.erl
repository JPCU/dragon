...
decrypt_value(Req,  <<Val/binary>>, scramble)   -> unscramble(Req#radius_request.secret, Req#radius_request.authenticator, Val);
...
-spec unscramble(secret(), authenticator(), binary()) -> binary().
unscramble(SharedSecret, RequestAuthenticator, <<PlainText/binary>>) ->
    B = crypto:hash(md5, [SharedSecret, RequestAuthenticator]),
    do_unscramble(SharedSecret, B, PlainText, << >>).

do_unscramble(SharedSecret, B, <<PlainText:16/binary, Remaining/binary>>, CipherText) ->
    NewCipherText = crypto:exor(PlainText, B),
    Bnext = crypto:hash(md5, [SharedSecret, PlainText]),
    do_unscramble(SharedSecret, Bnext, Remaining, <<CipherText/binary, NewCipherText/binary>>);

do_unscramble(_SharedSecret, _B, << >>, CipherText) ->
    CipherText.
...
scramble_dec_test() ->
    ?PLAIN_TEXT_PADDED = unscramble(?SECRET, ?REQUEST_AUTHENTICATOR, << ?ENC_PASSWORD >>).
